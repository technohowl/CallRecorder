...
public class RehearsalAudioRecorder
{
				...
				if (bSamples == 16)
				{
					...
					{ // 16bit sample size
						...
						if(mVolume != 1.0f) {
							...
							buffer.putShort(curSample);
							byte[] data = adjustVolume(buffer.array(), mVolume);
							curSample = ByteBuffer.wrap(data).order(ByteOrder.LITTLE_ENDIAN).getShort();*/
			...
			int audioFormat)
	{
		try
		{
			...
			{ // RECORDING_UNCOMPRESSED
				...
				try {
					aRecorder = new AudioRecord(aSource, sRate, nChannels, aFormat, bufferSize);
				} catch (IllegalArgumentException e) {
					e.printStackTrace();
					Log.e("CallRecorder","Switching to MIC");
					aRecorder = new AudioRecord(MediaRecorder.AudioSource.MIC, sRate, nChannels, aFormat, bufferSize);
	...
	public void setGain(double dBGain)
	{
		...
		Log.d("Gain", Double.toString(gain));
	}
	...
	public void start()
	{
		if (state == State.READY)
		{
			if (rUncompressed)
			{
				...
				{
					//noiseSuppressor = NoiseSuppressor.create(aRecorder.getAudioSessionId());
					if(AutomaticGainControl.isAvailable()) {
						Log.e("CallRecorder", "AutomaticGainControl available");

						//AutomaticGainControl.create(aRecorder.getAudioSessionId());
					}
